...
export function ParcelasCard({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {open && <AddParcelaForm onClose={() => setOpen(false)} />}
    </Card>
  )
}

function AddParcelaForm({ onClose }: { onClose: () => void }) {
  ...
  const valido = nome.trim().length > 0 && valor > 0 && total >= 1 && !!mesInicio
  ...
  return (
    <Modal title="Nova parcela" onClose={onClose}>
